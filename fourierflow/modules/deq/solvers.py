# Copied from https://github.com/locuslab/deq/blob/master/lib/solvers.py
...
def broyden(f, x0, threshold, eps=1e-3, stop_mode="rel", ls=False, name="unknown"):
    ...
    while nstep < threshold:
        ...
        u = (delta_x - matvec(part_Us, part_VTs, delta_gx)) / \
            torch.einsum('bij, bij -> b', vT, delta_gx)[:, None, None]
        vT[vT != vT] = 0
        u[u != u] = 0
        VTs[:, nstep-1] = vT
        Us[:, :, :, nstep-1] = u
...
def anderson(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-3, stop_mode='rel', beta=1.0, **kwargs):
    ...
    for k in range(2, threshold):
        ...
        alpha = torch.solve(y[:, :n+1], H[:, :n+1, :n+1]
                            )[0][:, 1:n+1, 0]   # (bsz x n)
        ...
        for mode in ['rel', 'abs']:
            if diff_dict[mode] < lowest_dict[mode]:
                if mode == stop_mode:
                    lowest_xest, lowest_gx = X[:, k % m].view_as(
                        x0).clone().detach(), gx.clone().detach()
                lowest_dict[mode] = diff_dict[mode]
