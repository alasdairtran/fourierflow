# Copied from https://github.com/locuslab/deq/blob/master/lib/jacobian.py
import numpy as np
import torch
...
def jac_loss_estimate(f0, z0, vecs=2, create_graph=True):
    ...
    for i in range(vecs):
        ...
        vJ = torch.autograd.grad(
            f0, z0, v, retain_graph=True, create_graph=create_graph)[0]
        result += vJ.norm()**2
...
def power_method(f0, z0, n_iters=200):
    ...
    for i in range(n_iters):
        vTJ = torch.autograd.grad(f0, z0, evector, retain_graph=(
            i < n_iters-1), create_graph=False)[0]
        evalue = (vTJ * evector).reshape(bsz, -1).sum(1, keepdim=True) / \
            (evector * evector).reshape(bsz, -1).sum(1, keepdim=True)
        evector = (vTJ.reshape(bsz, -1) / vTJ.reshape(bsz, -
                   1).norm(dim=1, keepdim=True)).reshape_as(z0)
