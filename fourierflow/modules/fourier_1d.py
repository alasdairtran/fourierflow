...
class SpectralConv1d(nn.Module):
    def __init__(self, in_dim, out_dim, n_modes, resdiual=True, dropout=0.1):
        ...
        fourier_weight = [nn.Parameter(torch.FloatTensor(
            in_dim, out_dim, n_modes, 2)) for _ in range(1)]
    ...
    def forward(self, x):
        ...
        x_ft = torch.fft.rfft(x, n=M, norm='ortho')
        # x_ft.shape == [batch_size, in_dim, grid_size, grid_size // 2 + 1]

        x_ft = torch.stack([x_ft.real, x_ft.imag], dim=3)
        # x_ft.shape == [batch_size, in_dim, grid_size, grid_size // 2 + 1, 2]

        out_ft = torch.zeros(B, I, M // 2 + 1, 2, device=x.device)
        # out_ft.shape == [batch_size, in_dim, grid_size, grid_size // 2 + 1, 2]

        out_ft[:, :, :self.n_modes] = self.complex_matmul_2d(
            x_ft[:, :, :self.n_modes], self.fourier_weight[0])

        out_ft = torch.complex(out_ft[..., 0], out_ft[..., 1])

        x = torch.fft.irfft(out_ft, n=M, norm='ortho')
        # x.shape == [batch_size, in_dim, grid_size, grid_size]
...
@Module.register('fourier_net_1d')
