...
class DepthWiseConv1d(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, padding=0, stride=1, bias=True, groups=False):
        ...
        self.net = nn.Sequential(
            nn.Conv1d(dim_in, dim_in, kernel_size=kernel_size,
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_out = nn.Sequential(nn.Linear(
            inner_dim, dim * 2), nn.GLU()) if on_attn else nn.Linear(inner_dim, dim)
    ...
    def forward(
        ...
        b, n, _, h, talking_heads, device, has_context = * \
            x.shape, self.heads, self.talking_heads, x.device, exists(context)
        ...
        if exists(rotary_pos_emb) and not has_context:
            ...
            (ql, qr), (kl, kr) = map(lambda t: (
                t[..., :l], t[..., l:]), (q, k))
            ql, kl = map(lambda t: apply_rotary_pos_emb(
                t, rotary_pos_emb), (ql, kl))
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.rel_pos = RelativePositionBias(scale=dim_head ** 0.5, causal=causal, heads=heads,
                                            num_buckets=rel_pos_num_buckets, max_distance=rel_pos_max_distance) if rel_pos_bias else None
    ...
    def forward(
        ...
        if exists(self.rotary_pos_emb):
            max_rotary_emb_length = max(
                *map(lambda m: (m.shape[1] if exists(m) else 0) + x.shape[1], mems))
...
class ContinuousTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.project_in = nn.Linear(dim_in, dim) if exists(
            dim_in) else nn.Identity()

        self.attn_layers = attn_layers
        self.norm = nn.LayerNorm(dim)

        self.project_out = nn.Linear(dim, dim_out) if exists(
...
class XTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        enc_transformer_kwargs = pick_and_pop(
            ['num_tokens', 'max_seq_len'], enc_kwargs)
        enc_transformer_kwargs['num_memory_tokens'] = enc_kwargs.pop(
            'num_memory_tokens', None)

        dec_transformer_kwargs = pick_and_pop(
            ['num_tokens', 'max_seq_len'], dec_kwargs)
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None):
        ...
        out = self.decoder(tgt, context=enc, mask=tgt_mask,
