...
class Fourier1DBurgersExperiment(Experiment):
    def __init__(self,
                 conv: Module,
    ...
    def _training_step(self, batch):
        ...
        B, X = x.shape
        # data.shape == [batch_size, X]

        pos_feats = self.encode_positions(
            [X], self.low, self.high, self.use_fourier_position)
        ...
        # Add positional info
        xx = rearrange(x, 'b m -> b m 1')
        xx = torch.cat([xx, pos_feats], dim=-1)
        out = self.conv(xx)
        im = out[0]
        im = rearrange(im, 'b m 1 -> b m')
